...
function ButtonGroupText({
	...
	return useRender({
		...
		props: mergeProps<"div">(
			{
				className: cn(
					"flex items-center gap-2 rounded-md border border-input bg-muted px-2.5 font-medium text-sm shadow-xs [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none",
					className
				)
			},
			props
