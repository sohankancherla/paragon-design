...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
	return (
		<th
			...
			className={cn(
				"h-9 whitespace-nowrap px-2 text-left align-middle font-medium text-muted-foreground text-xs [&:has([role=checkbox])]:pr-0",
				className
			)}
			{...props}
		/>
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
	return (
		<td
			...
			className={cn(
				"whitespace-nowrap p-2.5 align-middle [&:has([role=checkbox])]:pr-0",
				className
			)}
			{...props}
		/>
